# -*- coding: utf-8 -*-
"""
Created on Thu Apr  8 21:21:25 2021
...
for atom in Gn_s:
    if atom.index == 11:
        atom.symbol = 'S'
view(Gn_s)
write(originalPath + '/models/svs1c2_ini.cif', images = Gn_s)
...
for atom in Gn_s:
    if atom.index == 22:
        atom.symbol = 'S'
view(Gn_s)
write(originalPath + '/models/svs2c1_ini.cif', images = Gn_s)
...
for atom in Gn_s:
    if atom.index == 26:
        atom.symbol = 'S'
view(Gn_s)
write(originalPath + '/models/svs3_ini.cif', images = Gn_s)
...
for idx, TM in enumerate(TM_elements):
    if TM in target_elements:
        ...
        formula = TM + '_svs3'
        model = read(originalPath + '/models/svs3_ini.cif')
        model[24].symbol = TM

        model = sort(model)
        write(originalPath + '/models/%02d_%s_svs3.cif' % (idx + 1, TM), images = model)
