# -*- coding: utf-8 -*-
"""
Created on Thu Apr  8 19:58:31 2021
...
for atom in Gn_s:
    if atom.index == 5:
        atom.symbol = 'N'
# view(Gn_s)
write(originalPath + '/models/dvn1c3_ini.cif', images = Gn_s)
...
for atom in Gn_s:
    if atom.index == 24:
        atom.symbol = 'N'
# view(Gn_s)
write(originalPath + '/models/dvn2c2_ini.cif', images = Gn_s)
...
for atom in Gn_s:
    if atom.index == 21:
        atom.symbol = 'N'
# view(Gn_s)
write(originalPath + '/models/dvn3c1_ini.cif', images = Gn_s)
...
for atom in Gn_s:
    if atom.index == 9:
        atom.symbol = 'N'
# view(Gn_s)
write(originalPath + '/models/dvn4_ini.cif', images = Gn_s)
...
for idx, TM in enumerate(TM_elements):
    if TM in target_elements:
        ...
        formula = TM + '_dvn4'
        model = read(originalPath + '/models/dvn4_ini.cif')
        model[-1].symbol = TM

        model = sort(model)
        write(originalPath + '/models/%02d_%s_dvn4.cif' % (idx + 1, TM), images = model)
