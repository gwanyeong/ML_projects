...
with open('model_summary_slabs', 'w') as f:
     ...
     for idx in range(len(entries)):
        ...
        createFolder('%03d_%s/surface' % (idx + 1.0, formula))
    
        """
        Slab modeling
        """    
        bulk = read_vasp('%03d_%s/POSCAR' % (idx + 1.0, formula))
      
        slab = surface(bulk, (1,0,0), 3, vacuum = 7.5)  
...
#           print('%4.3f' % positions[i][2],end = '\t')
       ...
       #view(slab_sorted)
        ...
        INCAR
        """
        INCAR = Incar.from_file('%03d_%s/INCAR' % (idx + 1.0, formula))
        ...
        # Hubbard U setting
        elements = []
        for element in bulk.get_chemical_symbols():
...
#           print(slab_sorted[k].symbol," ",magmom[i])
        ...
        POTCAR
        """    
        potcar_ori = Potcar.from_file('%03d_%s/POTCAR' % (idx + 1.0, formula))
        ...
        Write files
        """
        write_vasp('%03d_%s/surface/POSCAR' % (idx + 1.0, formula), slab_sorted)
        INCAR.write_file('%03d_%s/surface/INCAR' % (idx + 1.0, formula))
        KPOINTS.write_file('%03d_%s/surface/KPOINTS' % (idx + 1.0, formula))     
        POTCAR.write_file('%03d_%s/surface/POTCAR' % (idx + 1.0, formula))

        # jobscript copy
        destination = '%03d_%s/surface/' % (idx + 1.0, formula)
