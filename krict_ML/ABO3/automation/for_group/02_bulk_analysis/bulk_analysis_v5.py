...
# Many error cases due to MP update
for i in range(len(df)):
    if df.formula[i] != sorted_entries[i]['pretty_formula']:
        print(i," ", df.formula[i]," ",sorted_entries[i]['pretty_formula'])
###############################################################################

entries = []
for idx in range(len(df)):
    for entry in entries_from_list:
        if entry['pretty_formula'] == df.formula[idx]:
            entries.append(entry)

# No error cases
for i in range(len(df)):
    if df.formula[i] != entries[i]['pretty_formula']:
        print(i," ", df.formula[i]," ",entries[i]['pretty_formula'])
    ...
    for idx in range(num):
        ...
        try:
            ...
            if (type(vbm) and type(cbm)) == float:
                f.writelines("%4.3f\t%4.3f\t%4.3f\t%4.3f\t%4.3f\t" % (entries[idx]['band_gap'], bg, vbm, e_fermi, cbm))
                f.writelines("%s\t%s\n" % (bgtype, bgpath))
            elif (vbm and cbm) == None:
                f.writelines("%4.3f\tNone\tNone\t%4.3f\tNone\t" % (entries[idx]['band_gap'], e_fermi))
                f.writelines("%s\t%s\n" % (bgtype, bgpath))
            ...
            f.writelines('%s/DOS files - unknown error\n' % (file_path))
            continue
