# -*- coding: utf-8 -*-
"""
Created on Sun Oct 18 16:24:42 2020
...
properties = ['formula',
              'convg_bare','tot_E_bare','DE_bare','DG_bare','bare_mag','nsteps_bare','time_bare',
              'convg_OOH','tot_E_OOH','DE_OOH','DG_OOH','OOH_mag','OOH_dmag','nsteps_OOH','time_OOH',
              'convg_O', 'tot_E_O','DE_O', 'DG_O','O_mag','O_dmag','nsteps_O','time_O',
              'convg_OH','tot_E_OH','DE_OH','DG_OH','OH_mag','OH_dmag','nsteps_OH','time_OH']
          ...
          # view(bare_opt)
            ...
            for k in range(len(adsorbates)):  
                try:
                    ...
                    idx_i = idx - num_ini
                    df.iloc[idx_i][8*k + 8] = v_slab.converged
                    df.iloc[idx_i][8*k + 9] = float('%4.4f' % E_slab)
                    ...
                    f.writelines('\tmagnetization: %4.3f\n' % mag_ads)
                    df.iloc[idx_i][8*k + 12] = float('%4.3f' % mag_ads)
                    df.iloc[idx_i][8*k + 13] = float('%4.3f' % dmag)
