...
with open('model_summary_adsorbates', 'w') as f:
    ...
    for idx in range(len(entries)):
        ...
        try:
            slab = read_vasp('%03d_%s/surface/POSCAR' % (idx + 1.0, formula))
            KPOINTS = Kpoints.from_file('%03d_%s/surface/KPOINTS' % (idx + 1.0, formula))
            potcar_ori = Potcar.from_file('%03d_%s/surface/POTCAR' % (idx + 1.0, formula))
 ...
 #              print(atom.index," ",atom.symbol," ",atom.position)
        ...
        for ads_idx in range(len(adsorbates)):
            try:
                INCAR = Incar.from_file('%03d_%s/surface/INCAR' % (idx + 1.0, formula))
            except FileNotFoundError:
                INCAR = Incar.from_file('%03d_%s/surface_np/cont/INCAR' % (idx + 1.0, formula))

            createFolder('%03d_%s/surface/%s' % (idx + 1.0, formula, adsorbate_names[ads_idx]))
...
#           view(slab_sorted)
            ...
            write_vasp('%03d_%s/surface/%s/POSCAR' % (idx + 1.0, formula, adsorbate_names[ads_idx]), slab_sorted)
           
            INCAR.write_file('%03d_%s/surface/%s/INCAR' % (idx + 1.0, formula, adsorbate_names[ads_idx]))
            KPOINTS.write_file('%03d_%s/surface/%s/KPOINTS' % (idx + 1.0, formula, adsorbate_names[ads_idx]))     
            POTCAR.write_file('%03d_%s/surface/%s/POTCAR' % (idx + 1.0, formula, adsorbate_names[ads_idx]))
        
            # jobscript copy
            destination = '%03d_%s/surface/%s/' % (idx + 1.0, formula, adsorbate_names[ads_idx])
