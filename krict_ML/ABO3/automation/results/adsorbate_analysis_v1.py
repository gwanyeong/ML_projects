...
properties = ['formula',
              'convg_bare','tot_E_bare','DE_bare','bare_mag','nsteps_bare',
              'convg_OOH','tot_E_OOH','DE_OOH','OOH_mag','OOH_dmag','nsteps_OOH',
              'convg_O', 'tot_E_O','DE_O', 'O_mag','O_dmag','nsteps_O',
              'convg_OH','tot_E_OH','DE_OH','OH_mag','OH_dmag','nsteps_OH']

###############################################################################

with open('results/analysis_summary_adsorbate', 'w') as f:
          ...
          # view(bare_opt)
            ...
            for k in range(len(adsorbates)):  
                try:
                    ...
                    df.iloc[idx][6*k + 6] = v_slab.converged
                    df.iloc[idx][6*k + 7] = float('%4.4f' % E_slab)
                    df.iloc[idx][6*k + 8] = float('%4.3f' % DE_ads)
                    ...
                    f.writelines('\tmagnetization: %4.3f\n' % mag_ads)
                    df.iloc[idx][6*k + 9] = float('%4.3f' % mag_ads)
                    df.iloc[idx][6*k + 10] = float('%4.3f' % dmag)
                    df.iloc[idx][6*k + 11] = len(v_slab.ionic_steps)
