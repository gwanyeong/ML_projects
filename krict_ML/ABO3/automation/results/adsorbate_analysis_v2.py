...
with open('results/results_summary', 'w') as f:
    ...
    df_mag = pd.DataFrame(np.array([['NaN' for p in range(len(magm_properties))] for q in range(num)]),
                          index = [r for r in range(1,num+1)],
    ...
    for idx in range(num):
        ...
        try:
            ...
            df_mag.magm_bare = str(metal_magm)
          ...
          # view(bare_opt)
            ...
            for k in range(len(adsorbates)):  
                try:
                    ...
                    df.iloc[idx][7*k + 7] = v_slab.converged
                    df.iloc[idx][7*k + 8] = float('%4.4f' % E_slab)
                    df.iloc[idx][7*k + 9] = float('%4.3f' % DE_ads)
                    ...
                    f.writelines('\tmagnetization: %4.3f\n' % mag_ads)
                    df.iloc[idx][7*k + 10] = float('%4.3f' % mag_ads)
                    df.iloc[idx][7*k + 11] = float('%4.3f' % dmag)
                    df.iloc[idx][7*k + 12] = len(v_slab.ionic_steps)

                    outcar_ads = Outcar(file_path + '%s/OUTCAR' % (adsorbates[k]))
                    time_ads = float(outcar_ads.run_stats['Total CPU time used (sec)'])/3600.0
                    df.iloc[idx][7*k + 13] = float('%4.3f' % time_ads)
                    ...
                    df_mag.iloc[idx][k+1] = str(metal_magm_ads)
...
#                   view(slab_opt)
    ...
    df.to_pickle('results/df_summary.pkl')

    df_mag.to_csv('results/df_mag.csv')
    df_mag.to_pickle('results/df_mag.pkl')
